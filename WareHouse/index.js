            ...
            prod.forEach((quantity, item) => {
                ...
                if (item == product) {
                    if (quantity >= 4) {
                        ...
                        // SET NEW QUANTITY LEFT AFTER SELL//
                        prod.set(item, quantityAfterSell)
                        this.getStokes()
                        // BUT IF QUANTITY IS LESS THAN 4 , THAN THE CLIENT NEED TO WAIT FOR NEW DELIVERY
                    } else {
                        console.log(`Dear client you need to wait for new delivery from the warehouse`);
                        this.takeGoodsFromWarehouse(wareHouse)
                    }
