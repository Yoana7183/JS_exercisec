...
class User {
    ...
    chooseRandomBook() {
        let diffrence = []
        diffrence = books.filter(element1 => {
            return !this.listOfBooks.find(element2 => {
...
class Librarian {
    ...
    processTheCustomerWhoseTurnCameFromTheQueue() {
        ...
        if (this.customers.length === 0) {
            throw new Error ('There is no more customers in the queue')
        }
    ...
    checkIfCustomerReachBooksLimit(customer) {
        ...
        if (customer.listOfBooks.length === 4) {
    ...
    getRandomBook(cust) {
        ...
        let freeBooks = cust.chooseRandomBook()

        const wantedBook = freeBooks[Math.floor(Math.random() * freeBooks.length)]
        return wantedBook
    ...
    //5. borrow book function - get curr book
    customerServiceForBorrowingABookFromTheLiibrary() {
        ...
        if (!this.checkIfCustomerReachBooksLimit(customerInOrder)) {
